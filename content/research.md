---
title: "Research"
---

## Work in Progress

<div class="paper">

### Long-Term Effects of Slum Upgrading on Climate Resilience

<span class="paper-draft-link">Draft (coming soon)</span> <a href="/primed-archives/" class="paper-draft-link">More about PRIMED</a>

<details class="paper-abstract">
<summary>Abstract</summary>
<p>This paper investigates the long-term effects of a large slum upgrading program on vulnerability to natural disasters in Medellín, Colombia. The program was implemented between 1993 and 2000 and primarily focused on infrastructure investments. Preliminary results suggest that slum upgrading reduces the probability of experiencing natural disasters and having housing damage, indicating that place-based investments can enhance climate resilience in these settings. Similarly, there exists suggestive evidence that the program increased housing density in upgraded areas characterized by lower natural disasters' risk, while non-upgraded slums continued developing in high risk land. I develop a spatial model to quantify general equilibrium effects of this policy and to analyze counter-factual scenarios. Preliminary results suggest that slum upgrading increases welfare all over the city. A key mechanism to explain these results is the improvement of local amenities derived from investments in local public infrastructure.</p>
</details>

</div>

<div class="paper">

### Rainwater Harvesting in Urban Settings as a Response to Water Shortages

<span class="paper-stage stage-scale-up">Scale Up Data Collection</span> <span class="authors-inline">With Paulina Oliva, Alejandro Lopez, Hernan Bejarano</span>

<details class="paper-abstract">
<summary>Abstract</summary>
<p>We will investigate the extent to which Rainwater Harvesting Systems (RWH) installed in households can reduce water insecurity in Mexico City, as well as the role of its regular maintenance in delivering these benefits. In our full-scale study, our first objective is to investigate whether ownership of a functioning RWH changes water consumption patterns, either by increasing the total amount of water consumed, or by reducing water demands from the network, or both. Our second objective is to investigate whether maintenance of the RWH plays a role in water capture and whether we can overcome the barriers to proper maintenance by providing training. In addition to differences in water quantity, we will measure water quality, time allocation (especially of women and youth), and expenses.</p>
</details>

</div>

<div class="paper">

### Spatial Misallocation and Supply–Demand Feedback: Evidence from the Boda-Boda Market in Kampala

<span class="paper-stage stage-exploring">Data Agreement Signed</span> <span class="authors-inline">With Mychaela Paetow, Santiago Di Martini</span>

</div>

<div class="paper">

### Informal Settlements and Natural Disaster Risk: Experimental Evidence from Medellín

<span class="paper-stage stage-exploring">Exploring</span>

</div>

<div class="paper">

### Decentralized Water Solutions for Rapidly Growing Cities: Evidence from Hargeisa

<span class="paper-stage stage-exploring">Exploring</span> <span class="authors-inline">With Paulina Oliva, Alejandro Lopez, Hernan Bejarano, Jingshu Chen</span>

</div>

## Published Policy Work

<div class="paper">

### <a href="https://publications.iadb.org/en/inclusive-cities-healthy-cities-all" target="_blank">Spatial Health Inequality in Latin American Cities: Magnitude, Drivers, and Policy Alternatives</a>

<span class="venue-inline">Inclusive Cities: Healthy Cities for All (2022). pp. 37–59. Inter-American Development Bank.</span>
<br><span class="authors-inline">With Juan Pablo Chauvin</span>

<details class="paper-abstract">
<summary>Abstract</summary>
<p>Spatial health inequities are pervasive across and within Latin American cities. This article explores the magnitude of spatial inequalities in health outcomes, their drivers, and policies that can be deployed to address them. Life expectancy, the burden of noncommunicable diseases, and, more recently, the impact of COVID-19, all exhibit large geographic disparities. These variations are correlated with individual-level socioeconomic factors, such as low schooling levels and high poverty rates. Nevertheless, spatial health inequalities are not only a reflection of the residential clustering of vulnerable individuals. Local environments themselves can improve or further deteriorate their communities' public health outcomes. These include weather conditions, public utilities, recreational infrastructure, and externalities—notably air and noise pollution. The current evidence suggests that existing policy tools available to local and national governments can be effective in creating healthier built environments, reducing and preventing negative externalities, and promoting healthier lifestyles in cities and neighborhoods.</p>
</details>

</div>
